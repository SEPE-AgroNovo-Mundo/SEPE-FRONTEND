...
const marcas = ref([
  'Seresto', 'Plasvale', 'Pet Games', 'Petmax', 'Chalesco'
])
const tipos = ref([
  'Cachorro', 'Gato'
])
const cores = ref([
  'Azul', 'Branco', 'Vermelho', 'Cinza', 'Prata'
])
const opcoes = ref([
  'Único', 'Médio'
])
...
const coresContagem = computed(() => {
  const contagem = {}
  cores.value.forEach(cor => {
    contagem[cor] = props.produtos.filter(p => p.cor === cor).length
  })
  return contagem
})
        ...
        <div class="filtro-bloco">
          ...
          <div class="filtro-lista">
            <label v-for="cor in cores.filter(c => c.toLowerCase().includes(corBusca.toLowerCase()))" :key="cor">
              <input type="checkbox" :value="cor" v-model="coresSelecionadas" /> {{ cor }} <span class="badge">{{
                coresContagem[cor] }}</span>
            </label>
          </div>
        </div>
        <div class="filtro-bloco">
